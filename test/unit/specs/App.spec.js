...
describe('App', () => {
  ...
  beforeEach(() => {
    ...
    STATE = vm.getStateHelper();
    // NOTE: PhantomJS doesn't support Audio Tags so we have to stub these
    //       2 methods that interacts w/ HTML5 Audio
  ...
  describe('Test Computed Properties', () => {
    ...
    it('[fractionOfTimeLeft] calculates the correct value in "WORK" mode', () => {
      ...
      for (let i = 0; i < statesToTest.length; i += 1) {
    ...
    it('[primaryButton] is computed correctly', () => {
      const startTimerSpy = sinon.spy(vm, 'startTimer');
      const pauseTimerSpy = sinon.spy(vm, 'pauseTimer');
  ...
  describe('Test Component Methods', () => {
    it('[startTimer] decrements [timeRemaining] every second in "WORK" mode', () => {
      ...
      for (let i = 0; i < statesToTest.length; i += 1) {
        ...
        expect(vm.timerWorker).to.equal(null);
    ...
    it('[startTimer] switches from "WORK" to "BREAK" mode when [timeRemaining] approaches 0', () => {
      const CLOCK = sinon.useFakeTimers();
      const spy = sinon.spy(vm, 'triggerAlarm');
    ...
    it('[switchToSettingsView] pauses timer', () => {
      const spy = sinon.spy(vm, 'pauseTimer');
    ...
    it('[switchToSettingsView] changes [showSettingsView] to true', () => {
      ...
      vm.switchToSettingsView();
      expect(vm.showSettingsView).to.equal(true);
      vm.switchToSettingsView();
      expect(vm.showSettingsView).to.equal(true);
    });
    ...
    it('[switchToMainView] updates variable based on $emit values', () => {
      ...
      expect(vm.allowNotification).to.equal(true);
    });

    it('[switchToMainView] resets timer', () => {
      const spy = sinon.spy(vm, 'resetTimer');
  ...
  describe('Test Child Components', () => {
    it('[radialBar] should render', () => {
      expect(vm.$el.querySelector('.radialBar')).to.not.equal(null);
    });

    it('[primaryButton] should render', () => {
      expect(vm.$el.querySelector('.primaryButton')).to.not.equal(null);
    });

    it('[resetButton] should render', () => {
      expect(vm.$el.querySelector('.resetButton')).to.not.equal(null);
    });
  });
});
